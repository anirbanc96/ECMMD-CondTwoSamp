...
K <- seq(5, 25, by = 5)
method_mat <- data.frame(method_list = c("SKCE", "derandomized_NN15", "derandomized_NN25", "asym_NN15", "asym_NN25"),
...
for (n in n_list) {
  ...
  for (signal in mag) {
    ...
    for (b in 1:B) {
      ...
      modified_predictor <- matrix(0, nrow = n, ncol = 2)
      modified_predictor[, 1] <- predictor[, 1] - predictor[, 1]^5
      modified_predictor[, 2] <- 1 - predictor[, 1]
      Y <- rbinom(n, 1, modified_predictor[, 1])
  ...
  # save the result
  saveRDS(p_value, sprintf("Power%d.rds", n))
